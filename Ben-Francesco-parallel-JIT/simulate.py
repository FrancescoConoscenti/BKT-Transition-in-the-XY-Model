...
def simulate_lattice(sim_params, algorithm):
    ...
    logger.info(f"L={L} ({algorithm}): Estimated BKT T_BKT ≈ {results['T_BKT']:.4f}")
...
def visualize_plots(results_dict, output_dir, algorithm):
    ...
    plot_props = {
        ...
        (1, 0): {'y': 'specific_heat', 'title': 'Specific Heat vs Temperature', 'ylabel': 'Specific Heat per Site', 'filename': 'specific_heat'},
        (1, 1): {'y': 'susceptibility', 'title': 'Susceptibility vs Temperature', 'ylabel': 'Susceptibility per Site', 'filename': 'susceptibility'},
        (2, 0): {'y': 'vortices', 'title': 'Vortex Density vs Temperature', 'ylabel': 'Vortex Density', 'filename': 'vortices'},
        (2, 1): {'y': 'stiffnesses', 'yerr': 'stiffness_errors', 'title': 'Spin Stiffness vs Temperature with BKT Transition', 'ylabel': 'Spin Stiffness', 'filename': 'stiffness'}
    }
