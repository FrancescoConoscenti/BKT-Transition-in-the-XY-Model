...
num_temps = 8  # Very few temperature points

# Ultra minimal simulation steps
simulation_steps = {
    'pretherm': 100,    # Minimal pre-thermalization
    'therm': 1000,       # Minimal thermalization
    'measure': 2000     # Minimal measurement
}
...
def run_analysis(L, force=False):
    ...
    # Skip if data already exists and force=False
    if os.path.exists(data_dir) and os.path.exists(f"{data_dir}/thermo_output.data") and not force:
    ...
    try:
        ...
        # Verify that thermo_output.data was created
        if not os.path.exists(f"./testL={L}finalData/thermo_output.data"):
            print(f"Warning: Analysis completed but thermo_output.data was not created for L={L}")
            return False
...
def load_data(L):
    ...
    thermo_file = f"{base_path}/thermo_output.data"
    variables_file = f"{base_path}/variables.data"
    
    # Print more detailed debug info for missing files
    ...
    if not os.path.exists(thermo_file):
        print(f"Thermo data file not found: {thermo_file}")
        # Try to run the analysis again to generate it
        print("Attempting to regenerate thermo data...")
        run_analysis(L, force=True)
        
        if not os.path.exists(thermo_file):
            raise FileNotFoundError(f"Unable to generate thermo data for L={L}")
...
def create_combined_figure(output_dir):
    ...
    for L in system_sizes:
        try:
            # Force running the analysis if needed
            analysis_path = f"./testL={L}finalData/thermo_output.data"
            if not os.path.exists(analysis_path):
                print(f"Thermo data not found for L={L}, running analysis...")
                run_analysis(L, force=True)
            ...
            print(f"Error processing data for L={L}: {str(e)}")
