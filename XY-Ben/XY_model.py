...
class XYSystem():
    def __init__(self,temperature = 3,width=10,algorithm="metropolis"):
        self.width = width
    ...
    def set_algorithm(self,algorithm):
        """Set the update algorithm to use (metropolis or wolff)"""
        self.algorithm = algorithm.lower()
    ...
    def sweep(self):
        if self.algorithm == "metropolis":
            ...
            raise ValueError(f"Unknown algorithm: {self.algorithm}. Use 'metropolis' or 'wolff'")
    
    def _metropolis_sweep(self):
        """Perform one sweep using the Metropolis algorithm"""
    ...
    def _wolff_sweep(self):
        """Perform one sweep using the Wolff cluster algorithm"""
        beta = 1.0 / self.temperature
        
        # Choose a random reflection vector (unit vector)
        r_angle = np.random.random() * 2 * pi
        r_vec = np.array([np.cos(r_angle), np.sin(r_angle)])
        
        # Choose a random seed spin
        seed = np.random.randint(0, self.num_spins)
        
        # Initialize cluster
        cluster = [seed]
        added = [False] * self.num_spins
        added[seed] = True
        ...
        while i < len(cluster):
            k = cluster[i]
            s_k = np.array([np.cos(self.spin_config[k]), np.sin(self.spin_config[k])])
            
            # Check neighbors
            for j in self.nbr[k]:
                if not added[j]:
                    s_j = np.array([np.cos(self.spin_config[j]), np.sin(self.spin_config[j])])
                    
                    # Project spins onto reflection axis
                    s_k_proj = np.dot(s_k, r_vec) * r_vec
                    s_j_proj = np.dot(s_j, r_vec) * r_vec
                    
                    # Calculate energy contribution from the projections
                    E_bond = -np.dot(s_k_proj, s_j_proj)
                    
                    # Probability to add to cluster
                    p_add = 1 - np.exp(min(0, 2 * beta * E_bond))
        ...
        # Reflect all spins in the cluster
        for idx in cluster:
            s = np.array([np.cos(self.spin_config[idx]), np.sin(self.spin_config[idx])])
            
            # Reflect s across r_vec (2(s·r)r - s)
            s_dot_r = np.dot(s, r_vec)
            s_reflect = 2 * s_dot_r * r_vec - s
            
            # Convert back to angle
            self.spin_config[idx] = np.arctan2(s_reflect[1], s_reflect[0]) % (2*pi)
