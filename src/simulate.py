...
def metropolis_sweep(spins, L, beta, J, neighbors):
    """Perform L**2 Metropolis spin updates using precomputed neighbors"""
    for _ in range(L * L):
...
def estimate_correlation_decay_parameters(correlation, T, L):
        """
        Estimate the critical exponent eta and correlation length xi from G(r)
        """
...
def wolff_sweep(spins, L, beta, J, neighbors):
    """Perform L**2 Wolff cluster updates using precomputed neighbors"""
    num_sites = L * L
...
def find_eta_cross(etas: np.ndarray, eta_errors: np.ndarray, temperatures: np.ndarray) -> tuple[float, float]:
    """
    Find crossing temperature where eta = 1/4 and estimate its propagated error.

    Parameters:
        etas (np.ndarray): Estimated eta values at each temperature
        eta_errors (np.ndarray): Jackknife error of eta at each temperature
        temperatures (np.ndarray): Temperature values

    Returns:
        T_eta_cross (float): Estimated temperature where eta = 1/4
    ...
    for i in range(len(etas) - 1):
        if (etas[i] - target_eta) * (etas[i + 1] - target_eta) < 0:
            ...
            # Average error on eta near crossing
...
def simulate_lattice(sim_params, algorithm):
    ...
    logger.info(f"L={L} ({algorithm}): Estimated BKT temperature from eta = 1/4 criterion (spin-spin correlation): T_BKT ≈ {results['T_BKT_eta']:.4f} ± {results.get('T_BKT_eta_error', float('nan')):.4f}")
    logger.info(f"L={L} ({algorithm}): Estimated BKT temperature from xi fit (correlation length): T_BKT ≈ {results['T_BKT_xi']:.4f} ± {results.get('T_BKT_xi_error', float('nan')):.4f}")
...
def visualize_plots(results_dict, output_dir, algorithm):
    ...
    plot_props = {
        ...
        (2, 1): {'y': 'etas', 'yerr': 'eta_errors', 'title': 'eta Exponent vs Temperature ', 'ylabel': 'Correlation Decay Exponent eta', 'filename': 'eta'},
        (2, 2): {'y': 'xis', 'yerr': 'xi_errors', 'title': 'Correlation Length xi vs Temperature', 'ylabel': 'Correlation Length xi', 'filename': 'xi'}
    }
    ...
    for L in lattice_sizes:
        ...
        # From eta exponent
        if not np.isnan(results['T_BKT_eta']):
           textstr += f"L = {L}: T_BKT (eta = 1/4) = {results['T_BKT_eta']:.4f}\n"
        else:
           textstr += f"L = {L}: T_BKT (eta = 1/4) = NaN\n"
        # From xi exponent
